...
1. **Name the destination** through a decision conversation that pins down what this map is finding its way to — the spec, decision, or change. The destination fixes the scope, so it's settled first. When the destination is itself a design or spec, run the fuller conversation with `wayfinder skill grill-with-docs`, which keeps a glossary and ADRs as you go. The protocol is the same either way:
...
3. Resolve it — **zoom as needed**: fetch the full body of any related or closed ticket on demand; invoke the skills the `## Notes` block names. If in doubt, run the decision conversation — `wayfinder skill grill-with-docs`.
